...
export default function AnosConosco() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const records = await pb.collection('tempo_empresa').getFullList<Homenagem>({ sort: '-created' });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await pb.collection('tempo_empresa').delete(id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedRecord = await pb.collection('tempo_empresa').update(id, formData);
      setHomenagens((prev) =>
  ...
  return isLoading ? (
    ...
    <div className="container mx-auto p-6 min-h-screen">
      <h1 className="text-4xl font-bold text-center mb-10">Tempo de Casa</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {homenagens.map((homenagem) => (
          <Card key={homenagem.id} className="relative overflow-hidden">
            <div className="relative aspect-[4/3]">
              <Image
                src={`${pocketBaseUrl}/tempo_empresa/${homenagem.id}/${homenagem.thumbnail}`}
                alt={homenagem.nome}
                layout="fill"
                objectFit="cover"
            ...
            <div className="p-4">
              ...
              <Badge className="mt-2">tempo_de_empresa</Badge>
